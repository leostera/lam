...
PerformMove:
  if move.dst[1] = "global" /\ move.src[1] = "literal" then
    global_registers[move.dst[2]] := move.src[2]
  elsif move.dst[1] = "local" /\ move.src[1] = "literal" then
    local_registers[move.dst[2]] := move.src[2]
  elsif move.dst[1] = "global" /\ move.src[1] = "global" then
  ...
  Run:
    while current_count > 0 do
      ...
      with src \in Registers,
           ...
           lit \in Literals do
        assert (\/ /\ current_move.dst[1] = "global"
                  /\ current_move.src[1] = "literal"
                  /\ global_registers[move.dst[2]] = current_move.src[2]
               \/ /\ current_move.dst[1] = "local"
                  /\ current_move.src[1] = "literal"
                  /\ local_registers[move.dst[2]] = current_move.src[2]
               \/ /\ current_move.dst[1] = "global"
                  /\ current_move.src[1] = "local"
                  /\ global_registers[move.dst[2]] = local_registers[current_move.src[2]]
               \/ /\ current_move.dst[1] = "local"
                  /\ current_move.src[1] = "local"
                  /\ local_registers[move.dst[2]] = local_registers[current_move.src[2]]
               \/ /\ current_move.dst[1] = "global"
                  /\ current_move.src[1] = "global"
                  /\ global_registers[move.dst[2]] = global_registers[current_move.src[2]]
               \/ /\ current_move.dst[1] = "local"
                  /\ current_move.src[1] = "global"
                  /\ local_registers[move.dst[2]] = global_registers[current_move.src[2]]
                  ) = TRUE;
        if use_lit then
...
\* BEGIN TRANSLATION (chksum(pcal) = "c2085bc" /\ chksum(tla) = "7eb4ed8f")
...
\* Last modified Sat Dec 19 14:26:09 CET 2020 by ostera
